-- BVM Design Center — Initial Schema
-- Tables: clients, builds, messages, notifications
...
create index if not exists idx_clients_assigned_rep on clients(assigned_rep);
...
create table if not exists builds (
  ...
  qa_report jsonb,
  edited_html text,
  editor_status text not null default 'generated',
  qa_edited_score integer,
  qa_edited_at timestamptz,
  qa_hash text,
...
create index if not exists idx_builds_assigned_dev on builds(assigned_dev);
...
do $$ begin
  ...
  if not exists (select 1 from pg_policies where policyname = 'Allow all on builds') then
  ...
  if not exists (select 1 from pg_policies where policyname = 'Allow all on pulse_timers') then
