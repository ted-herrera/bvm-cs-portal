...
// Maps subType + businessType to one of the three locked templates
// (or "custom" when the ask exceeds what the templates cover).

export type DefaultTemplate = "local" | "community" | "premier" | "custom";

const SUBTYPE_TO_TEMPLATE: Record<string, DefaultTemplate> = {
  // Premier — restaurants, retail, beauty, med spa, sushi, nightlife
  mexican: "premier",
  pizza: "premier",
  burger: "premier",
  bakery: "premier",
  ...
  // Local — trades, home services, cleaning, auto, landscaping
  // (most default to local via businessType)

  // Community — professional services, fitness, education
  martialarts: "community",
  yoga: "community",
  crossfit: "community",
  dance: "community",
  pilates: "community",
...
const BUSINESSTYPE_TO_TEMPLATE: Record<string, DefaultTemplate> = {
  restaurant: "premier",
  beauty: "premier",
  retail: "premier",
...
export function getDefaultTemplate(
  subTypeOrBusinessType: string,
  maybeBusinessType?: string,
): DefaultTemplate {
  const subType = (subTypeOrBusinessType || "").toLowerCase();
  if (SUBTYPE_TO_TEMPLATE[subType]) return SUBTYPE_TO_TEMPLATE[subType];
  const businessType = (maybeBusinessType || subTypeOrBusinessType || "").toLowerCase();
  if (BUSINESSTYPE_TO_TEMPLATE[businessType]) return BUSINESSTYPE_TO_TEMPLATE[businessType];
  return "community";
}
