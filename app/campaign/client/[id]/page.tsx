...
export default function CampaignClientPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const initMap = useCallback(() => {
    ...
    function buildMap() {
      ...
      // Approximate coordinates from ZIP (use a simple geocoding lookup)
      // For now center on approximate US city coordinates
  ...
  async function saveTagline() {
    ...
    try {
      ...
      console.error("Tagline save error:", e);
    }
  ...
  async function sendRepMessage() {
    ...
    try {
      await fetch(`/api/campaign/revision/${id}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ note: repMessage, type: "message" }),
      });
      setRepMsgSent(true);
      setRepMessage("");
  ...
  // Income ring breakdown (simulated from SBR data)
  const incomeRings = [
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "40px 24px 80px" }}>
        ...
        <Section id="market-report">
          ...
          <div
            ref={mapRef}
            style={{
              height: 320, borderRadius: 12, background: "#0d1a2e",
              border: "1px solid rgba(255,255,255,0.08)", marginBottom: 28, overflow: "hidden",
            }}
          />

          {/* Income Ring Breakdown */}
          <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 16, color: "#fff", margin: "0 0 16px" }}>
            Income Distribution by Ring
          </h3>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12, marginBottom: 28 }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 10, padding: 16, borderLeft: "3px solid #3B82F6" }}>
              ...
              <div style={{ fontSize: 14, color: "#fff", fontWeight: 600 }}>
                {competitorGap || "Opportunity to own this market"}
              </div>
            </div>
            <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 10, padding: 16, borderLeft: "3px solid #8B5CF6" }}>
              ...
              <div style={{ fontSize: 14, color: "#fff", fontWeight: 600 }}>
                {marketBrief || `${client.business_name} is positioned to lead in ${client.city}`}
              </div>
            </div>
          </div>
        ...
        {/* ── YOUR CAMPAIGN ──────────────────────────────────────────────────── */}
        <Section>
          <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 24, color: "#fff", margin: "0 0 24px" }}>
            Your Ad Direction
          </h2>
          <div style={{ display: "flex", gap: 28, alignItems: "flex-start", flexWrap: "wrap" }}>
            <div style={{ position: "relative", flexShrink: 0 }}>
              {approvedDir?.imageUrl ? (
                ...
                <div style={{ width: 320, height: 320, background: "rgba(255,255,255,0.04)", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(255,255,255,0.2)" }}>
                  No image
                </div>
              )}
              <span style={{
                position: "absolute", top: 12, right: 12,
                background: "rgba(245,200,66,0.9)", color: "#1B2A4A",
            ...
            <div style={{ flex: 1, minWidth: 220 }}>
              <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, color: "#fff", margin: "0 0 12px" }}>
              ...
              {/* Tagline */}
              {client.tagline ? (
                <p style={{ fontSize: 18, color: "#F5C842", fontStyle: "italic", fontFamily: "'Playfair Display', serif", margin: "0 0 20px" }}>
                ...
                <div style={{ marginBottom: 20 }}>
                  <input
                    ...
                    style={{
                      ...
                      borderRadius: 6, padding: "10px 14px", fontSize: 14, color: "#fff", outline: "none", boxSizing: "border-box",
                    }}
                  />
                  <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                    <button onClick={saveTagline} disabled={taglineSaving || !taglineInput.trim()} style={{
                      background: "#F5C842", color: "#1B2A4A", border: "none", borderRadius: 6,
                      padding: "8px 16px", fontSize: 12, fontWeight: 700, cursor: "pointer",
                    }}>{taglineSaving ? "Saving..." : "Save"}</button>
                    <button onClick={() => setEditingTagline(false)} style={{
                ...
                <button onClick={() => setEditingTagline(true)} style={{
                  ...
                  fontSize: 14, fontWeight: 700, cursor: "pointer", padding: 0, marginBottom: 20,
                }}>
                  Add Your Tagline →
                </button>
              )}
              ...
              {/* Status */}
              <div style={{
                background: client.stage === "production" ? "rgba(245,158,11,0.1)" : "rgba(34,197,94,0.1)",
                border: `1px solid ${client.stage === "production" ? "rgba(245,158,11,0.2)" : "rgba(34,197,94,0.2)"}`,
              ...
              {approvedDir?.imageUrl && (
                <a href={approvedDir.imageUrl} download={`${client.business_name}-campaign.png`} style={{
                  ...
                  border: "1px solid rgba(255,255,255,0.15)", borderRadius: 8, padding: "8px 18px",
          ...
          <div style={{ marginTop: 32, borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 24 }}>
            <h3 style={{ fontSize: 16, color: "#fff", margin: "0 0 12px", fontWeight: 700 }}>
              ✏️ Request a Change
            </h3>
            <textarea
              value={revisionNote}
              ...
              style={{
                ...
                border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, padding: 14,
                fontSize: 14, color: "#fff", resize: "vertical", outline: "none", boxSizing: "border-box",
              }}
            />
            <button
              ...
              style={{
                marginTop: 8, background: revisionNote.trim() ? "#F5C842" : "rgba(255,255,255,0.08)",
                color: revisionNote.trim() ? "#1B2A4A" : "rgba(255,255,255,0.3)",
                border: "none", borderRadius: 8, padding: "10px 24px", fontSize: 14,
                fontWeight: 700, cursor: revisionNote.trim() ? "pointer" : "not-allowed",
              }}
            >
        ...
        <Section>
          <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 24, color: "#fff", margin: "0 0 4px" }}>
            See Your Complete BVM Campaign
          </h2>
          <p style={{ fontSize: 14, color: "rgba(255,255,255,0.45)", margin: "0 0 4px" }}>
            This is what a full print + digital + web campaign looks like for {client.business_name}.
          </p>
          <p style={{ fontSize: 11, color: "rgba(255,255,255,0.25)", margin: "0 0 28px" }}>
            Your current campaign includes {client.ad_size} print. The preview below shows the complete suite.
          </p>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 20, marginBottom: 28 }}>
            ...
            <div style={{ background: "rgba(245,200,66,0.04)", border: "2px solid rgba(245,200,66,0.3)", borderRadius: 14, overflow: "hidden" }}>
              <div style={{ height: 200, background: "#0d1a2e", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
                {approvedDir?.imageUrl ? (
                  <img src={approvedDir.imageUrl} alt="Print Ad" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                ) : (
                  <div style={{ color: "rgba(255,255,255,0.15)", fontSize: 13 }}>Preview</div>
                )}
              </div>
          ...
          <div style={{ textAlign: "center" }}>
            {upsellSent ? (
              <div style={{ background: "rgba(34,197,94,0.1)", border: "1px solid rgba(34,197,94,0.2)", borderRadius: 10, padding: "16px 24px", display: "inline-block" }}>
                <p style={{ fontSize: 15, color: "#22c55e", fontWeight: 700, margin: 0 }}>
                  Your rep has been notified — expect a call soon! 🎉
                </p>
              </div>
            ) : (
              <button
                onClick={sendUpsellInterest}
                ...
                style={{
                  ...
                  boxShadow: "0 4px 16px rgba(245,200,66,0.3)",
                  opacity: upsellSending ? 0.6 : 1,
                }}
              >
                {upsellSending ? "Sending..." : "Talk to Your Rep About the Full Suite →"}
              </button>
            )}
          </div>
        </Section>
        ...
        {/* ── YOUR REP ───────────────────────────────────────────────────────── */}
        <Section>
          <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 24, color: "#fff", margin: "0 0 20px" }}>
            Your Rep
          </h2>
          <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 20 }}>
            <div style={{ width: 52, height: 52, borderRadius: "50%", background: "#F5C842", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, color: "#1B2A4A", fontSize: 18 }}>
              TH
            </div>
            <div>
          ...
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 16 }}>
            <h3 style={{ fontSize: 14, fontWeight: 700, color: "#fff", margin: "0 0 12px" }}>Send a Message</h3>
            <textarea
              value={repMessage}
              onChange={(e) => setRepMessage(e.target.value)}
              placeholder="Type your message to Ted..."
              style={{
                width: "100%", minHeight: 80, background: "rgba(255,255,255,0.04)",
                border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, padding: 14,
                fontSize: 14, color: "#fff", resize: "vertical", outline: "none", boxSizing: "border-box",
            ...
            <button
              ...
              disabled={!repMessage.trim() || repMsgSending}
              style={{
                marginTop: 8, background: repMessage.trim() ? "#F5C842" : "rgba(255,255,255,0.08)",
                color: repMessage.trim() ? "#1B2A4A" : "rgba(255,255,255,0.3)",
                border: "none", borderRadius: 8, padding: "10px 24px", fontSize: 14,
                fontWeight: 700, cursor: repMessage.trim() ? "pointer" : "not-allowed",
              }}
            >
              {repMsgSending ? "Sending..." : "Send Message"}
            </button>
            {repMsgSent && <p style={{ fontSize: 13, color: "#22c55e", margin: "8px 0 0" }}>✓ Message sent — Ted will get back to you shortly.</p>}
          </div>
        </Section>
      </div>
    </div>
